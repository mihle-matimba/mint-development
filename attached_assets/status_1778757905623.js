  ...
  try {
    ...
    if (req.headers.authorization?.startsWith("Bearer ")) {
      ...
      if (user) {
        ...
        console.log(`[Sumsub] auth header present but invalid for ${userId}: ${authErr}`);
      }
    }
    ...
    if (db) {
      try {
        ...
        if (packRecord) {
          ...
          // Sumsub → profiles writeback for already-verified users (gated on auth)
          if (authedUserId === userId) {
            try {
              const { data: packFull } = await db
                .from("user_onboarding_pack_details")
                .select("pack_details")
                .eq("user_id", userId)
                .maybeSingle();
